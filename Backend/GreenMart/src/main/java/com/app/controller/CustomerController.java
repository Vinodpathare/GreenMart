...
public class CustomerController {
	...
	@Autowired
	private OrdersDetailsRepository orderRepo;
	@Autowired
	...
	//OrderDetails order=orderRepo.findById(orderId).get();
     // System.out.println(order);
		...
		try {
			int Order =custService.addOrdersDetails(userId, orderId);
			Orders orders =  ordersrepo.findById(orderId).get();
			User user=orders.getSelectedCustomer();
			List<OrderDetails> od=orderRepo.findBySelectedOrder(orders);
		//List<OrderDetails> items=orders.getSelectedCustomer().getOrderDetails();
			...
			text.append("We recieved your order, this order will be deliverd within a 48 hour on adderss.\n"
					 + "\nOrder details are as follows\n");
			od.forEach((detail) -> {
				text.append("Product name: " + detail.getProductName()+ ",\n");
				text.append("Quantity: " + detail.getQty() + ",\n");
				text.append("Price : Rs " + detail.getFinalPrice()*detail.getQty() + "\n");
			});
Double totalPrice=od.stream().mapToDouble(x -> x.getFinalPrice()*x.getQty()).sum();
			
			text.append("Your Order Id:"+orderId+"\n");
			text.append("Total Amount:"+totalPrice+"\n");
			text.append("Thanks for choosing GreenMart...!!\n");
			String message = text.toString();
			msg.setText(message);
			mailsender.send(msg);
			
			return new ResponseDTO<>(HttpStatus.OK, "order Details Added",Order );
		}catch (RuntimeException e) {
